...
class Geometry:
    ...
    def isCCW(p1: Point, p2: Point, p3: Point) -> int:
        ...
        returns 0 if p1, p2, p3 are collinear,
                -1 if p1 -> p2 -> p3 -> p1 is mathematically CCW,
                +1 if p1 -> p2 -> p3 -> p1 is mathematically CW.
        """
        val = (p2.x - p1.x) * (p3.y - p2.y) - (p2.y - p1.y) * (p3.x - p2.x)
        if val == 0:            
            return 0
        
        # Standard Cartesian CCW yields positive determinant
